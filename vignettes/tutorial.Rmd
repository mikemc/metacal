...
* Axes transformations: The multiplicative error created by bias can only be
  ...
  Note the use of pseudo-log-transformed axes. I will saved these axes
  transformations in a list (allow with `coord_fixed()`) so that we can reuse
...
which expect two matrices or phyloseq objects with the observed and actual
...
0. We will therefore use add a pseudocount of 1 to the observed counts to
...
error across the full range of predicted proportions, though it is
systematically underpredicting the observed relative abundances at the lowest
values.
...
To verify calibration that worked as expected, let's compare the observed,
...
model fit, but seen from a slightly different angle. We can see that
calibration in this dataset clearly reduces the relative error in proportions
across the full range of tested true proportions, but leads to estimates that
are systematically too low at low proportions by a small factor.
